...
from schemas import Waybill, Task, IntermediateTaskResultIn
from api import API
...
async def stop_continue_task(message: Message, state: FSMContext):
    ...
    task = Task(**data['task'])
    current_task = await get_task_by_id(message, token, waybill.id, task.id)
    stop_resp = await API.stop_continue_task(token, waybill.id, current_task.id, not current_task.isFinished)
...
async def start_deleting_result(message: Message, state: FSMContext):
    ...
    if not current_task:
        return
    await message.answer('Введите номер результата, который хотите удалить',
...
async def delete_result(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer('Введите номер существующего результата')
        return
...
async def start_adding_task(message: Message, state: FSMContext):
    await message.answer('Введите время прибытия <b>строго</b> в следующем формате: ЧЧ:ММ',
...
async def enter_transported(message: Message, state: FSMContext):
    ...
    request = IntermediateTaskResultIn(arrivalTime=time, distance=distance, idTask=task.id, transported=message.text)
