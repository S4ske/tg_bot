...
from src.dto.schemas import AnswerType, Checklist, Question, Waybill
from src.dto.states import Menu
from src.utils import validate_response
from telegram.beauty_print import print_checklist
from telegram.keyboards import (
    checkbox_markup,
    default_checklist_markup,
    get_keyboard_markup_buttons,
    get_keyboard_markup_checklists,
    waybill_markup,
)
    ...
    if question_num == 0:
        ...
        if current_question.answerType == AnswerType.checkbox:
            await message.answer(
                current_question.question, reply_markup=checkbox_markup
            )
        elif current_question.answerType == AnswerType.button:
            await message.answer(
                current_question.question,
                reply_markup=get_keyboard_markup_buttons(
                    current_question.answerParam.buttonArr
                ),
            )
        else:
            await message.answer(
                current_question.question, reply_markup=default_checklist_markup
            )
...
async def get_checklists(message: Message, state: FSMContext):
    ...
    await message.answer(
        f'Контрольные листы для путевого листа "{waybill.govNumber} <u>{waybill.date[:10]}":</u>',
...
async def get_questions(message: Message, state: FSMContext):
    ...
    try:
        checklist = list(
            filter(lambda x: x.id == int(message.text), checklists_resp.get)
        )[0]
    except IndexError:
        await message.answer("Введите номер существующего контрольного листа")
        return
    await message.answer(
        f'Вы готовы к прохождению контролього листа "{checklist.name}"?',
        reply_markup=ReplyKeyboardMarkup(
            keyboard=[[KeyboardButton(text="Начать")], [KeyboardButton(text="Назад")]]
        ),
    )
...
async def validate_answer(
    message: Message, state: FSMContext, current_question
) -> bool:
    data = await state.get_data()
    token = data["token"]
    waybill = Waybill(**data["waybill"])
    ...
    if (
        current_question.answerType == AnswerType.checkbox
        and message.text.casefold() not in ["да", "нет"]
    ):
        await message.answer("Введите Да/Нет")
        return False
    elif current_question.answerType == AnswerType.button and message.text not in list(
        map(lambda x: x.name, current_question.answerParam.buttonArr)
    ):
        await message.answer("Данного ответа нет в списке")
        return False
    answer_resp = await API.put_checklist_answer(
        token,
        waybill.id,
        checklist.checklistWaybillId,
        message.text,
        current_question.id,
    )
...
async def stop_checklist(message: Message, state: FSMContext) -> bool:
    ...
    waybill = Waybill(**data["waybill"])
    checklist = Checklist(**data["checklist"])
    stop_resp = await API.stop_checklist(
        token, waybill.id, checklist.checklistWaybillId
    )
