...
class TestUtils:
    ...
    @pytest.mark.parametrize(
        "is_success, exception_message, result, answer_text",
        [
            (True, None, True, None),
    ...
    async def test_validate_response_with_not_none(
        self,
        is_success: bool,
        ...
        validation_res = await utils.validate_response(
            self.message, response, exception_message
        )
    ...
    async def test_validate_response_with_none(self, exception_message: str | None):
        validation_res = await utils.validate_response(
            self.message, None, exception_message
        )
    ...
    @pytest.mark.parametrize(
        "task_id, get_tasks_response, is_valid, contains",
        [
            ...
            (
                ...
                Mock(
                    TaskResponse,
                    task=[Mock(Task, id=2), Mock(Task, id=1), Mock(Task, id=3)],
                    success=True,
                ),
                True,
                True,
            ...
            (
                ...
                Mock(
                    TaskResponse,
                    task=[Mock(Task, id=1), Mock(Task, id=3)],
                    success=True,
                ),
                True,
                False,
    ...
    async def test_get_task_by_id(
        ...
        mock,
        task_id: int,
        get_tasks_response: TaskResponse,
        is_valid: bool,
        contains: bool,
    ):
        mock.return_value = get_tasks_response
        result = await utils.get_task_by_id(
            self.message, "doesn't_matter", 1337, task_id
        )
        if is_valid:
            if contains:
